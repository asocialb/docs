...
In the IM account system, the password required by a user login is calculated by the server with a key provided by IM. For more information, see [Generating UserSig](https://intl.cloud.tencent.com/document/product/1047/34385). In the development phase, to avoid holding back development on the client, you can also calculate the `UserSig` in the [console](https://console.tencentcloud.com/im/tool-usersig) as shown below:

![](https://staticintl.cloudcachetci.com/cms/backend-cms/81e77873498711ef8357525400bdab9d.png)

- **Configuring an admin account**

During live streaming, an admin may need to send messages to a live room and mute (remove) non-compliant users, which can be done through APIs on the IM server as described in [RESTful API List](https://www.tencentcloud.com/document/product/1047/34621). To call these APIs, you need to [create an IM admin account](https://console.tencentcloud.com/im/account-management). By default, IM provides an account with the `UserID` of `administrator`. You can also create multiple admin accounts as needed. Note that you can create up to five admin accounts.

- **Configuring the callback address and enabling the callback**

To implement lucky draws based on on-screen comments, message statistics collection, sensitive content detection, and other requirements, you need to use the IM callback module, where the IM backend calls back the business backend in certain scenarios. You only need to provide an HTTP API and configure it in the [Webhook configuration](https://console.tencentcloud.com/im/callback-setting) module in the console as shown below:

![](https://staticintl.cloudcachetci.com/cms/backend-cms/61d2693c498811ef818752540055f650.png)

### Step 3. Integrate the client SDK

After completing the preparations, you need to integrate the IM and TRTC client SDKs into your project. You can select different integration options as needed. For detailed directions, see [Get Started](https://www.tencentcloud.com/document/product/1047/60367).
...
> **Note:**The IM SDK for web allows users to join only one audio-video group (`AVChatRoom`) at a time. If a user logs in to an client and enters live room A, multi-client login is enabled in the [console](https://console.tencentcloud.com/im/login-message), and the user logs in to another client and enters live room B, the user will be removed from live room A.
...
When the host recommends an item, it needs to be immediately displayed in the item slot at the bottom of the screen and notified to all viewers. Notifications of new items are generally triggered by the virtual assistant. We recommend you implement notifications of new items by enabling the admin to modify a custom group field as follows:
  ...
  3.1. **Add a custom group field**
    3.1.1. Log in to the [IM console](https://console.tencentcloud.com/im), click the target IM application card, and select **Configuration** > **Group configuration**> **Custom Group Field** on the left sidebar.
    3.1.2. On the **Custom Group Field** page, click **Add** in the top-right corner.

![](https://staticintl.cloudcachetci.com/cms/backend-cms/680babe6498911ef998b525400f69702.png)

    3.1.3. In the pop-up dialog box, enter a field name and set the group types and read/write permissions.

> **Note:**The field name can contain up to 16 characters, supporting letters, digits, and underscores (_). It cannot begin with a digit.A custom group field and a custom group member field cannot have the same name.![](https://staticintl.cloudcachetci.com/cms/backend-cms/c6db5e93498911ef998b525400f69702.png)

    3.1.4. Select **I understand that after a custom group field is added, only the read-write permissions of the added group type can be modified; the group type cannot be reselected or deleted; the field cannot be deleted.** and click **Confirm**.

> **Note:**The custom group field will take effect in about ten minutes after configuration.

  3.2. **Use the custom group field**
...
To implement more live room features such as user identity, user level, historical messages, and displaying the number of online users, see [Live Room Setup Guide](https://www.tencentcloud.com/en/document/product/1047/49589#2.-live-room-feature-development-guide).


---
*Source: [https://www.tencentcloud.com/document/product/267/51161](https://www.tencentcloud.com/document/product/267/51161)*
