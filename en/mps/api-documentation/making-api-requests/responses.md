...
, which contains
RequestId
, as long as it processes the request. It does not matter if the request is successful or not.
RequestId is the unique ID of an API request.  Contact us with this ID when an exception occurs.
Except for the fixed fields, all fields are action-specified. For the definitions of action-specified fields, see the corresponding API documentation. In this example,
TotalCount
and
InstanceStatusSet
are the fields specified by the API
DescribeInstancesStatus
. 0
TotalCount
means that the requester owns 0 CVM instance so the
InstanceStatusSet
is empty.
...
The presence of the
Error
field indicates that the request has failed. A response for a failed request will include
Error
,
Code
and
...
fields.
Code
is the code of the error that helps you identify the cause and solution. There are two types of error codes so you may find the code in either common error codes or API-specified error codes.
...
---
*Source: [https://www.tencentcloud.com/document/product/1041/33631](https://www.tencentcloud.com/document/product/1041/33631)*
