...
The app admin can send messages from the backend, or impersonate other users to send messages. In the latter case, even if the app admin or sender is not a group member, the messages will still be delivered.

### App admin simulates system messages

The app admin can simulate system notification by sending messages from the backend. These messages are delivered as system notification to specified online group members, and the app adminâs custom messages that are received on the app can be handled in special ways.

## SEQ Mechanism of Group Messages

Chat maintains a message SEQ for each group, which starts from an initial value of 1. Every time an ordinary message is sent in the group chat, the Chat backend uses the current SEQ value as the SEQ of the message and then increases the SEQ by 1.

Group ID and SEQ together constitute the unique identifier of a message.

> **Caution:** Chat generates incremental SEQs only for roamable messages.
...
> **Caution:**If the sender nickname and profile photo need to be included, import the information in these two fields to user profiles in Chat.If custom fields need to be included, configure custom fields in the console and submit a ticket to add the corresponding fields to messages.
...
> **Note:**Number-based frequency control limits the maximum number of messages sent by all client APIs per second in a single group. The default value is 40 messages per second. When the number of sent messages exceeds the limit, the backend will first deliver higher-priority messages, with messages with the same priority delivered randomly.

A message that has been restricted by frequency control is not delivered or stored in the message history, but a success response will be returned to the sender. [Before Group Message Is Sent](https://intl.cloud.tencent.com/document/product/1047/34374) webhook is triggered, but [After Group Message Is Sent](https://intl.cloud.tencent.com/document/product/1047/34375) webhook is not triggered.
...
1.2 The system updates the group message information, including the latest group message seq.
...
4. The SDK automatically pulls recent contacts and triggers the `OnNewMessage` API.
...
---
*Source: [https://trtc.io/document/33526](https://trtc.io/document/33526)*
