...
This API is used by the app admin to reset custom group attributes.
...
| v4/group_open_http_svc/set_group_attr | Request API |
...
- **Basic format**
Reset the custom attributes of a group:

```
{  "GroupId": "@TGS#aC5SZEAEF",  "GroupAttr":[      {          "key":"attr_key", //Attribute key          "value":"attr_val" //Attribute value      }ï¼      {          "key":"attr_key1",          "value":"attr_val1"      }  ]}
```
...
| --- | --- | --- | --- |
| GroupId | String | Yes | The ID of the group for which you want to reset custom attributes |
| GroupAttr | Array | Yes | List of custom attributes. `key`: Key of the custom attribute, which can be up to 32 bytes in length. Up to 16 keys are supported. `value`: Value of the custom attribute, which can be up to 4,000 bytes in length. |
...
---
*Source: [https://trtc.io/document/44190](https://trtc.io/document/44190)*
