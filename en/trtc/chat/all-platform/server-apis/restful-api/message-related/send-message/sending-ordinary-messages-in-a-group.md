...
> **Noticeï¼**When a user sends a message, the sender's avatar and nickname are automatically embedded in the message. This allows the recipient to use this information directly without having to request the account details from the server again after receiving the message, thereby avoiding API rate limiting and improving user experience. If additional user information is required, it is recommended to embed it into the message payload and send it along with the message.
...
```
{  "GroupId": "@TGS#2C5SZEAEF",  "From_Account": "leckie", // Message sender (optional)  "Random": 8912345, // A random number. If the random numbers of two messages are the same within five minutes, they are considered to be the same message.  "MsgBody": [ // Message body, which consists of an element array. For details, see the field description.      {          "MsgType": "TIMTextElem", // Text          "MsgContent": {              "Text": "red packet"          }      },      {          "MsgType": "TIMFaceElem", // Emoji          "MsgContent": {              "Index": 6,              "Data": "abc\\u0000\\u0001"          }      }  ]}
```
...
> **Note**This feature is supported only in the Pro edition ãPro Plus editionãEnterprise edition.

```
{  "GroupId":"@TGS#12DEVUDHQ",  "Random":2784275388,  "MsgBody":[      {          "MsgType":"TIMCustomElem",          "MsgContent":{              "Data":"1cddddddddq1"          }      }  ],  "To_Account":["brennanli2", "brennanli3"] // Specify the message recipient (up to 50 recipients can be specified). If this field is used, the message is excluded from the unread count.}
```

- **Specifying that messages do not trigger conversation update**
If `SendMsgControl` is set to `NoLastMsg`, it indicates not to trigger conversation update; if it is set to `NoUnread`, it indicates not to include the message in the unread count.
...
The mentioned users (@) set in the `GroupAtInfo` field have a one-to-one and sequential correspondence to the mentioned @ users in the message body.

> **Caution**Audio-video groups (AVChatRoom) don't support the @all feature.
...
| OfflinePushInfo | Object | No | Information of offline push. For more information, see [OfflinePushInfo Description](https://www.tencentcloud.com/document/product/1047/77715). |
...
| To_Account | Array | No | Specify up to 50 message receivers. If this field is used, the message will be excluded from the unread count. It is available only in the Pro edition ãPro Plus editionãEnterprise edition for work groups (Work), public groups (Public), and meeting groups (Meeting). |
...
| NeedReadReceipt | Integer | No | Whether this message requires a read receipt. 0: No (default), 1: Yes. |
| GroupAtInfo | Array | No | The @ notification of this message.âGroupAtAllFlag: 1 indicates @all; 0 indicates specific members.âGroupAtUserList: Specifies the UserIDs of the @ members.` |
...
[After a Group Message Is Sent](https://intl.cloud.tencent.com/document/product/1047/34375)


---
*Source: [https://trtc.io/document/34959](https://trtc.io/document/34959)*
