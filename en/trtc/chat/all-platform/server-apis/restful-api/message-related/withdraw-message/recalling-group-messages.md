...
- To recall group messages sent from a client, enable [After a Group Message Is Sent](https://intl.cloud.tencent.com/document/product/1047/34375) first, which records the `GroupId` and `MsgSeq` of each group message, and then enter the `GroupId` and `MsgSeqList` obtained from the webhook in this API to recall the message. Alternatively, get information of group messages to be recalled with [Pulling Historical Messages](https://intl.cloud.tencent.com/document/product/1047/34971), and then recall the message with this API.
...
- Once a message is recalled by this API, it is recalled from the local caches of the senderâs and recipientsâ clients.
...
Use the [RESTful API online debugging tool](https://tcc.tencentcs.com/im-api-tool/index.html#/v4/group_open_http_svc/group_msg_recall) to debug this API.
...
---
*Source: [https://trtc.io/document/34965](https://trtc.io/document/34965)*
