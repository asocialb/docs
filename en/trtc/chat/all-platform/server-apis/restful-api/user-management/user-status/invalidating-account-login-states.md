...
For example, when a developer finds a malicious account, the developer can call this API to invalidate the userâs login state. When the user uses the original UserSig state to log in to Chat, the login fails.

> **Note:**When an administrator uses this API to invalidate a userâs login status, all previous UserSig values for that user will expire. For details on client-side behavior, see the [login](https://im.sdk.qcloud.com/doc/zh-cn/classcom_1_1tencent_1_1imsdk_1_1v2_1_1V2TIMManager.html#a73fc0e14c5f2f5fc06a80081479fb416) documentation.After a userâs login state is invalidated using this API, the user can use a newly generated UserSig state to log in to Chat. This API can invalidate only one account at a time.
...
---
*Source: [https://trtc.io/document/34957](https://trtc.io/document/34957)*
