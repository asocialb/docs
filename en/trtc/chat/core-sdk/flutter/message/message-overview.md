...
In the Tencent Cloud Chat SDK, the message class is `V2TimMessage` ([Details](https://pub.dev/documentation/tencent_cloud_chat_sdk/latest/models_v2_tim_message/V2TimMessage-class.html)), which will be frequently used for message sending and receiving.
...
| nickName | Nickname of the message sender | It can be set by users and modified by calling `setSelfInfo`. For more information, see [User Profile](https://intl.cloud.tencent.com/document/product/1047/48160). |
...
| needReadReceipt | Whether the message requires a read receipt | It is set by the sender and valid for the Ultimate edition on v6.1 or later. For more information, see [Read Receipt](https://pub.dev/documentation/tencent_cloud_chat_sdk/latest/manager_v2_tim_message_manager/V2TIMMessageManager/sendMessageReadReceipts.html). |
| priority | Message priority | It applies only to group messages and can be set by calling the `sendMessage` API. |
| groupAtUserList | List of users tagged in the group message | It applies only to group messages and stores the `userID`. For more information, see [createAtSignedGroupMessage method](https://comm.qq.com/im/doc/flutter/en/SDKAPI/Api/V2TIMMessageManager/createTextAtMessage.html). |
...
| textElem | Text message storage element | It needs to be created and entered when a text message is sent. When the message is received and `elemType` is identified as `TEXT`, the content can be parsed from `textElem`. For more information, see [sendMessage method](https://pub.dev/documentation/tencent_cloud_chat_sdk/latest/manager_v2_tim_message_manager/V2TIMMessageManager/sendMessage.html) / [onRecvNewMessage property](https://pub.dev/documentation/tencent_cloud_chat_sdk/latest/enum_callbacks/OnRecvNewMessageCallback.html). |
...
| mergerElem | Merged message storage element | It needs to be created and entered when a merged message is sent. When the message is received and `elemType` is identified as `MERGER`, the content can be parsed from `mergerElem`. For more information, see [createForwardMessage method](https://pub.dev/documentation/tencent_cloud_chat_sdk/latest/models_v2_tim_merger_elem/V2TimMergerElem-class.html). |
| groupTipsElem | Group tip message storage element | Currently, a group tip message can be sent only by the server. When the message is received by the SDK and `elemType` is identified as `GROUP_TIPS`, the content can be parsed from `groupTipsElem`. For more information, see [onRecvNewMessage property](https://pub.dev/documentation/tencent_cloud_chat_sdk/latest/enum_callbacks/OnRecvNewMessageCallback.html). |
...
---
*Source: [https://trtc.io/document/47991](https://trtc.io/document/47991)*
