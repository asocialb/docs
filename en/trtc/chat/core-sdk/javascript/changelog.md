...
- [searchCloudUsers](https://www.tencentcloud.com/document/product/1047/67679#b57f11f2-d80b-4e40-b9e3-8f9463195624), enabling the search for cloud-based users.
- [searchCloudGroups](https://www.tencentcloud.com/document/product/1047/67681#b57f11f2-d80b-4e40-b9e3-8f9463195624), facilitating the search for cloud-based group lists.
- [searchCloudGroupMembers](https://www.tencentcloud.com/document/product/1047/67683#b57f11f2-d80b-4e40-b9e3-8f9463195624)ï¼ allowing for the search of cloud-based group member lists.

### 3.4.9 @2024.12.6

**New Features**

- Support for IPV6; Mini Program platforms should add the legitimate socket domain `wss://wssv6.im.qcloud.com`.
...
- Volume optimization, reduced by 27KB.
- Optimized `TOTAL_UNREAD_MESSAGE_COUNT_UPDATED` event notification.
...
- Multi-instance signaling timeout synchronization issue.

### 3.2.2 @2023.11.23

**Added**

- Supports sub-applications under the same `SDKAppID`.
- `createForwardMessage` supports forwarding `JSON` serialized `message`.
- Ensure multi-end consistency of session state after weak network recovery.
- Ensure the success rate of `createGroup` or `joinGroup`.

**Fix**

- Live group online member list, `GroupMember.isOnline` default value is `false`.
- Using `ChatCore` + `group-module.js` integration method, `message` prototype chain pointing issue.
- Other known issues.
...
- `getLoginUser` returns the userID of the logged-in user if logged in, otherwise returns `''`.
- Support for the Indonesia site.
- Optimized the experience of fetching group roaming messages when there are a large number of group designated messages or hollow messages.

**Fix**

- Using the REST API `mark_contact` to set standard/custom Definition session tags simultaneously does not trigger an SDK session list update.
- Using the REST API `group_msg_recall` to recall live group messages does not trigger the `MESSAGE_REVOKED` event in the SDK.

### 3.2.0 @2023.9.13

**Added**

- addMessageReaction adds message reactions (requires the flagship version). This interface supports scenarios such as "emoji reactions", for example:
...
- `removeMessageReaction` removes message reactions.
- `getMessageReactions` batch retrieves multiple message reactions.
- `getAllUserListOfMessageReaction` paginates to pull the user list of specified message reactions.
- `getGroupMemberList` supports retrieving the group member list including online status (requires the flagship version).
- `getGroupOnlineMemberCount` supports querying the online member count for all group types.

**Fix**

- Pulling group roaming cloud response packets can't continue with relay for directed messages only.
- The friend request list data was unexpectedly cleared.
- In some scenarios, previously retrieved group roaming messages were unexpectedly cleared.

### 3.1.3 @2023.8.21

**Added**

- `convertVoiceToText` supports voice-to-text conversion.
...
- `setConversationDraft`, supports setting conversation drafts.
- Supports the international site applications using the local review feature.

### 3.0.0 @2023.6.25

Released [@tencentcloud/chat](https://www.npmjs.com/package/@tencentcloud/chat), upgraded SDK to version V3.[Integration Guide](https://web.sdk.qcloud.com/im/doc/v3/zh-cn/tutorial-03-integration.html).
...
- Added [markGroupMemberList](https://web.sdk.qcloud.com/im/doc/en/SDK.html#markGroupMemberList) API to mark a member of an audio-video group (Premium edition required).
...
- Supported getting the list of online members of an audio-video group. For details, see [getGroupMemberList](https://web.sdk.qcloud.com/im/doc/en/SDK.html#getGroupMemberList) (Premium edition required).
- Supported blocking a member of an audio-video group. For details, see [deleteGroupMember](https://web.sdk.qcloud.com/im/doc/en/SDK.html#deleteGroupMember) (Premium edition required).
...
- Added [markConversation](https://web.sdk.qcloud.com/im/doc/en/SDK.html#markConversation) to mark a conversation (Premium edition required).
- Added [getConversationGroupList](https://web.sdk.qcloud.com/im/doc/en/SDK.html#getConversationGroupList) to get the list of conversation groups (Premium edition required).
- Added [createConversationGroup](https://web.sdk.qcloud.com/im/doc/en/SDK.html#createConversationGroup) to create a conversation group (Premium edition required).
- Added [deleteConversationGroup](https://web.sdk.qcloud.com/im/doc/en/SDK.html#deleteConversationGroup) to delete a conversation group (Premium edition required).
- Added [renameConversationGroup](https://web.sdk.qcloud.com/im/doc/en/SDK.html#renameConversationGroup) to rename a conversation group (Premium edition required).
- Added [addConversationsToGroup](https://web.sdk.qcloud.com/im/doc/en/SDK.html#addConversationsToGroup) to add a conversation to a conversation group (Premium edition required).
- Added [deleteConversationsFromGroup](https://web.sdk.qcloud.com/im/doc/en/SDK.html#deleteConversationsFromGroup) to delete a conversation from a conversation group (Premium edition required).
...
- Added the `nick` and `nameCard` parameters to the `lastMessage` data structure of [Conversation](https://web.sdk.qcloud.com/im/doc/en/Conversation.html) to better display the senderâs information of the `lastMessage` in a group chat.
...
- After an audio-video group (AVChatRoom) was created in the [console](https://console.tencentcloud.com/im) and a group owner was specified, messages sent by the RESTful API for [Sending System Messages in a Group](https://intl.cloud.tencent.com/document/product/1047/34958) would be repeated on the group owner side after the group owner joined the group.
...
- [Message](https://web.sdk.qcloud.com/im/doc/en/Message.html) adds the `namecard` attribute to display group membersâ group name cards (i.e., their nicknames in a group).
...
**Changes**[Error codes](https://web.sdk.qcloud.com/im/doc/en/global.html) are reduced and optimized.

**Bug fixing**

- After an audio-video group was created in the [console](https://console.tencentcloud.com/im) and a group owner was specified, messages sent by other group members will be repeated on the group owner side after the group owner joins the group.
- When groups were created and terminated in the [console](https://console.tencentcloud.com/im) or using a RESTful API frequently, the SDK did not deliver the [TIM.EVENT.GROUP_SYSTEM_NOTICE_RECEIVED](https://web.sdk.qcloud.com/im/doc/en/module-EVENT.html#.GROUP_SYSTEM_NOTICE_RECEIVED) event.
...
**New features**[Message priorities](https://intl.cloud.tencent.com/document/product/1047/33526), [enumerated values](https://web.sdk.qcloud.com/im/doc/en/module-TYPES.html#.MSG_PRIORITY_HIGH), and [use cases](https://web.sdk.qcloud.com/im/doc/en/SDK.html#createTextMessage) can be set for group messages.
...
**Changes**[getUserProfile](https://web.sdk.qcloud.com/im/doc/en/SDK.html#getUserProfile) and [updateMyProfile](https://web.sdk.qcloud.com/im/doc/en/SDK.html#updateMyProfile) support [custom profile fields](https://intl.cloud.tencent.com/document/product/1047/33520).
...
**New features**[getGroupList](https://web.sdk.qcloud.com/im/doc/en/SDK.html#getGroupList) supports pulling group profile information, including the group owner ID and group member count.
...
**Bug fixing**[getMessageList](https://web.sdk.qcloud.com/im/doc/en/SDK.html#getMessageList) could still pull historical messages in a quit group after rejoining.
...
---
*Source: [https://trtc.io/document/34281](https://trtc.io/document/34281)*
