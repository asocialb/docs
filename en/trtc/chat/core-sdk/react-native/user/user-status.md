...
The React Native SDK provides a feature for User Status Management, where each user has two different types of status:
...
- Offline (`TencentCloudChat.TYPES.USER_STATUS_OFFLINE`): Web login/logout will not trigger offline status. In apps integrating Native ChatSDK, offline status will be triggered.
...
## ![](https://cloudcache.intl.tencent-cloud.com/cms/backend-cms/0de73078c8be11ef86f2525400bf7822.png)Set your custom status

You can call the interface `setSelfStatus` to set the `customStatus` field for your custom status. Once set successfully, you will receive a notification of your status change through the `TencentCloudChat.EVENT.USER_STATUS_UPDATED` event. For details, please refer to [the "Status Change Notification"](https://www.tencentcloud.com/document/product/1047/66156#7118302a-bb4e-4104-9d2b-75b9962a4b77) below.
...
Based on the type of users whose status you wish to sense, the status change can be divided into three types:

1. Sensing one's own status change.

2. Sensing the status change of friends.

3. Sensing the status change of users (non - friends).

The status change notifications of the above three methods are all returned through the callback of onUserStatusChanged (Java / Swift / Objective - C / C++). However, different types of users trigger this notification in different ways.
...
---
*Source: [https://trtc.io/document/66156](https://trtc.io/document/66156)*
