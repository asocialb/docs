...
`SDKAppID` is the unique ID that the Chat service uses to identify a customer account. We recommend you apply for a new `SDKAppID` for every independent app to automatically isolate messages between `SDKAppIDs`.
You can view all `SDKAppIDs` in the [Chat console](https://console.tencentcloud.com/im) or click **Create Application** to create an `SDKAppID`.

### Setting the SdkConfig

It is used to configure the storage path of Chat running logs and data.

### sdk_config_config_file_path

Storage path of local Chat data.

> **Caution** The app needs to have read-write access to this path.

### sdk_config_log_file_path

It is the storage path of the Chat logs.
...
> **Caution** If you receive the [`UserSigExpiredCallback`](https://comm.qq.com/im/doc/unity/zh/callback/UserSigExpiredCallback.html) callback, the `UserSig` that you use for login has expired. In this case, you need to use the newly issued `UserSig` to log in again. If you continue to use the expired `UserSig`, the Chat SDK will enter an infinite login loop.
...
### 1. You must initialize the Chat SDK before using the login, message, group, conversation, relationship chain and profile, and signaling features.


---
*Source: [https://trtc.io/document/48570](https://trtc.io/document/48570)*
