...
- Introduced C API support for fetching read timestamps in group application lists.
- Introduced C API support for batch marking group application lists as read.
- Resolved an issue where synchronized conversation markers could be lost during login if the local conversation record was missing.
- Fixed a potential failure when updating atAll data in conversation info during multi-device login sessions.
- Fixed abnormal behavior when fetching merged message lists after locally inserting merged messages.
- Fixed an issue that prevented pulling nested merged messages while offline.
...
- Added support for returning operator information to pinned messages.
...
- Fixed the issue of the pinned message list returning in the wrong order.
- Fixed the issue of incorrect parsing of the Tips type in pinned messages.
- Fixed the issue of log writing failure on some Android phones.
- Fixed the issue of occasionally incomplete pulling of group roaming messages from old to new.
- Fixed the issue of occasionally failing to pull local messages when pulling historical messages from topics.
- Fixed the issue where sessions deleted in a conversation group are reactivated after logging in again.
...
- Fix the issue of data type error in Swift SDK location messages.
- Fix the occasional issue of triggering **message update** callback when pulling historical messages.
- Fix the issue of topic `at` message exception.
...
- Optimized Room control logic
- Solved the problem that the SDK cannot receive notifications of conversation deletion from the group when the local conversation does not exist
- Solved the problem that a conversation's last message can still be searched when it is in a withdrawn state
- Solved the problem that message senders cannot receive message change callbacks after live group messages are modified by third-party callbacks
- Fixed occasional instability problems in the log module
- Optimized community topic unread count logic

## 7.7.5282 @2023.12.18 - Enhanced Version

### SDK

- Add V2TIMCommunityManager and V2TIMCommunityListener to integrate community topic interfaces
- SDK supports Quic plugin
- Optimized conversation filtering logic
- CreateTime field is added in the topic information
- Live group supports setting administrators and pulling administrator list
- The regular expressions of local content moderation ignore English case by default
- Fixed occasional jni issue
- Fixed occasional callback error issue of weak network message sending
- Fixed occasional intValue change issue fo group tips message
- Fixed search failure issue for English issue mark by Android SDK

### TUIKit & Demo

- Added ChatBot plugin for intelligent chat, supporting FAQ question-and-answer format and streaming text message display
- TUIKit adapts to Gradle 8.0
- Simplified TUIKit component initialization steps and increase component dependency configuration
- TUIChat supports half screen horizontal display in RoomKit
- TUIChat supports listeners such as click events and long press events for adding messages
...
- Fixed the occasional issue of inaccurateÂ isReadÂ status of messages when users sent and received messages again after a one-to-one chat is deleted.
...
- Changed enumerated values ofÂ V2TIMGroupApplicationGetTypeÂ to those ofÂ V2TIMGroupApplicationType
- Changed the attribute nameÂ getTypeÂ ofÂ V2TIMGroupApplicationÂ toÂ applicationType
...
- Fixed the issue that senderâs profile photo is empty in the callback for inserting a local message successfully.
...
- Fixed the occasional crashes when the topic list was obtained.
- Fixed the exception in getting the conversation list after a topic was deleted.
...
- Added the feature of displaying 20 historical messages from before a user joins an audio-video group (AVChatRoom). This feature is available only for the Pro edition and Pro  Plus dition ãEnterprise  edition .
...
- Supported communities of up to 100,000 members. This feature is available only for the Pro edition and Pro Plus editionãEnterprise edition.
...
- Optimized the logic of receiving audio-video group messages. When an audio-video group receives a message, the senderâs nickname and profile photo are no longer queried.
...
- Fixed the issue where there was no callback when you tried to get the list of groups that you joined when you hadnât joined any group.
...
- Optimized the group profile pull logic. For audio-video groups, usersâ own group member information does not need to be pulled.
...
---
*Source: [https://trtc.io/document/34282](https://trtc.io/document/34282)*
