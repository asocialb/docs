...
- **Skin Smoothing Adjustment**: Configure smoothing intensity (0â9).
- **Whitening Adjustment**: Configure whitening intensity (0â9).
- **Rosy Adjustment**: Configure rosy intensity (0â9).
...
- **Video Live Streaming**: Please refer to [Quick Start](https://www.tencentcloud.com/document/product/647/74593) to integrate AtomicXCore and complete the setup of LiveCoreView..
- **Voice Chat Room**: Please refer to [Quick Start](https://www.tencentcloud.com/document/product/647/74681) to integrate AtomicXCore and complete the setup of LiveCoreView.
...
```
class BeautyManager() {    private val baseBeautyStore = BaseBeautyStore.shared()        // Set smoothing level (input range 0â100, internally converted to 0â9)    fun updateSmoothLevel(uiLevel: Int) {        val sdkLevel = (uiLevel / 100.0f * 9.0f)        baseBeautyStore.setSmoothLevel(sdkLevel)        println("Set smoothing level: UI=$uiLevel, SDK=$sdkLevel")    }    // Set whitening level (input range 0â100, internally converted to 0â9)    fun updateWhitenessLevel(uiLevel: Int) {        val sdkLevel = (uiLevel / 100.0f * 9.0f)        baseBeautyStore.setWhitenessLevel(sdkLevel)        println("Set whitening level: UI=$uiLevel, SDK=$sdkLevel")    }    // Set rosy level (input range 0â100, internally converted to 0â9)    fun updateRuddyLevel(uiLevel: Int) {        val sdkLevel = (uiLevel / 100.0f * 9.0f)        baseBeautyStore.setRuddyLevel(sdkLevel)        println("Set rosy level: UI=$uiLevel, SDK=$sdkLevel")    }}
```
...
```
class BeautyManager() {    private val baseBeautyStore = BaseBeautyStore.shared()    // Reset all basic beautification effects    fun resetBeautyEffects() {        baseBeautyStore.reset()    }}
```
...
| `BaseBeautyStore` | Basic beautification: adjust smoothing, whitening, and rosy (levels 0â9), reset status, synchronize effect parameters. | [API Documentation](https://tencent-rtc.github.io/TUIKit_Android/-atomic-x%20-core%20-a-p-i/io.trtc.tuikit.atomicxcore.api.device/-base-beauty-store/index.html) |
...
---
*Source: [https://trtc.io/document/74605](https://trtc.io/document/74605)*
