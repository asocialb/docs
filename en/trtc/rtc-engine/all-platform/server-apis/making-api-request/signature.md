...
The Signature is generated by the security credentials which include SecretId and SecretKey. If you don't have the security credentials yet, go to the [TencentCloud API Key](https://console.tencentcloud.com/capi) page to apply for them; otherwise, you cannot invoke the TencentCloud API.

## 1. Applying for Security Credentials

Before using the TencentCloud API for the first time, go to the [TencentCloud API Key](https://console.tencentcloud.com/capi) page to apply for security credentials.
...
HTTP method: POST and GET modes are supported, and GET is used here for the request. Please note that the method name should be in all capital letters.
Request server: the domain name of the request to view the list of instances (DescribeInstances) is cvm.tencentcloudapi.com. The actual request domain name varies by the module to which the API belongs. For more information, see the instructions of the specific API.
Request path: The request path in the current version of TencentCloud API is fixed to /.
...
---
*Source: [https://trtc.io/document/34265](https://trtc.io/document/34265)*
