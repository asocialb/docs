...
`TRTCKaraokeRoom` is an open-source class that depends on two closed-source Tencent Cloud SDKs. For the specific implementation process, see [Karaoke (Android)](https://intl.cloud.tencent.com/document/product/647/41941).

- The [TRTC SDK](/document/product/647/35078) is used as a low-latency audio chat component.
- The `AVChatRoom` feature of the [Chat SDK](https://intl.cloud.tencent.com/document/product/1047) is used to implement chat rooms. The attribute APIs of Chat are used to store room information such as the seat list, and invitation signaling is used to send requests to speak or invite others to speak.
...
| [setSpeaker](#setspeaker) | Sets whether to use the device speaker or receiver to play audio. |
...
| [onUserVolumeUpdate](#onuservolumeupdate) | The user volume. |
...
This API is used to terminate a [TRTCKaraokeRoom](https://intl.cloud.tencent.com/document/product/647/41941) singleton object.
...
This API is used to set the event callbacks of [TRTCKaraokeRoom](https://intl.cloud.tencent.com/document/product/647/41941). You can use `TRTCKaraokeRoomDelegate` to get different status notifications of [TRTCKaraokeRoom](https://intl.cloud.tencent.com/document/product/647/41941).
...
1. A user calls `createRoom` to create a karaoke room, passing in room attributes (i.e., room ID, whether listeners need room ownerâs permission to speak, number of seats).
...
> **Note**After a user becomes a speaker, all users in the room will receive an `onSeatListChange` notification and an `onAnchorEnterSeat` notification.
...
---
*Source: [https://trtc.io/document/41943](https://trtc.io/document/41943)*
