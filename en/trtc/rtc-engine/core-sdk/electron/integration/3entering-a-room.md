...
| --- | --- | --- | --- | --- |
| SDKAppID | Application ID | You can view your application ID in the [TRTC console](https://console.tencentcloud.com/trtc/app). If you donât have an application yet, click **Create application** to create one. | Number | 1400000123 |
...
> **Note**In TRTC, a user cannot use the same `userId` to enter the same room on two different devices at the same time; otherwise, there will be a conflict.The value of `appScene` must be the same on each client. Inconsistent `appScene` may cause unexpected problems.
...
---
*Source: [https://trtc.io/document/48049](https://trtc.io/document/48049)*
