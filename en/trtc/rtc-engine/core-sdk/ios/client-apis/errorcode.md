...
| EnumType | DESC |
| --- | --- |
| [TXLiteAVError](https://www.tencentcloud.com/document/product/647/35135#e9c6eb6577e24853dd9716de29044384) | Error Codes. |
| [TXLiteAVWarning](https://www.tencentcloud.com/document/product/647/35135#f29be1160857221275e6fc415e54100e) | Warning codes. |
...
| ERR_INVALID_PARAMETER | -2 | An invalid parameter was passed in when the API was called. |
| ERR_REFUSED | -3 | The API call was rejected. |
| ERR_NOT_SUPPORTED | -4 | The current API cannot be called. |
| ERR_INVALID_LICENSE | -5 | Failed to call the API because the license is invalid. |
...
| ERR_SCREEN_CAPTURE_START_FAIL | -1308 | Failed to start screen recording. It occurs on a mobile device, it may be because the user denied screen sharing permission, it occurs on Windows or macOS, check whether the parameters of the screen recording API are set as required. |
| ERR_SCREEN_CAPTURE_UNSURPORT | -1309 | Screen recording failed. Screen recording is only supported on Android versions later than 5.0 and iOS versions later than 11.0. |
...
| ERR_SYSTEM_LOOPBACK_START_FAILED | -1307 | System audio loopback startup encountered an error. Common error reasons and solutions are: 1. The current system version does not support the include or excluded specified process loopback function, which requires Windows system version 10.0.19042 (20H2) or higher. For x64 specified path loopback function, it also requires Windows system version 10.0.19042 (20H2) or higher. Please check the current system version. 2. Invalid input parameters (including empty parameters, non-existent input paths, non-existent process IDs, and non-existent device names). Please check whether the input parameters are legal. 3. No available speaker device, please check whether it is available. |
| ERR_SYSTEM_LOOPBACK_THIRD_PARTY_APP_EXIT | -1324 | The third party app loopback capture encountered an error. This error occurred after the third party app loopback was successfully started. During the sound collection process, the specified process or application unexpectedly exited. Please check the status of the specified process or application and try again by re-entering the process number |
| ERR_TRTC_ENTER_ROOM_FAILED | -3301 | Failed to enter the room. For the reason, refer to the error message for -3301 in ` onError `. |
| ERR_TRTC_REQUEST_IP_TIMEOUT | -3307 | IP and signature request timed out. Check your network connection and whether your firewall allows UDP.Try visiting the IP address 162.14.22.165:8000 or 162.14.6.105:8000 and the domain default-query.trtc.tencent-cloud.com:8000. |
...
| ERR_TRTC_ROOM_PARAM_NULL | -3316 | Empty room entry parameters. Please check whether valid parameters were passed in to the ` enterRoom:appScene: ` API. |
| ERR_TRTC_INVALID_SDK_APPID | -3317 | Incorrect room entry parameter. Check whether ` TRTCParams.sdkAppId ` is empty. |
| ERR_TRTC_INVALID_ROOM_ID | -3318 | Incorrect room entry parameter. Check whether ` TRTCParams.roomId ` or ` TRTCParams.strRoomId ` is empty. Note that you cannot set both parameters. |
...
| ERR_TRTC_ENTER_ROOM_REFUSED | -3340 | Request to enter room denied. Check whether you called ` enterRoom ` twice to enter the same room. |
| ERR_TRTC_INVALID_PRIVATE_MAPKEY | -100006 | Advanced permission control is enabled but failed to verify ` TRTCParams.privateMapKey `.For details, see [Enabling Advanced Permission Control](https://intl.cloud.tencent.com/document/product/647/35157). |
...
| ERR_TRTC_USER_SIG_CHECK_FAILED | -100018 | Failed to verify ` UserSig `. Check whether ` TRTCParams.userSig ` is correct or valid.For details, see [UserSig Generation and Verification](https://intl.cloud.tencent.com/document/product/647/39074). |
...
| ERR_TRTC_PUSH_THIRD_PARTY_CLOUD_FAILED | -3323 | Abnormal response packets for relay. |
| ERR_TRTC_MIX_TRANSCODING_FAILED | -3324 | Abnormal response packet for On-Cloud MixTranscoding. |
...
| ERR_TRTC_CONNECT_OTHER_ROOM_TIMEOUT | -3326 | The co-anchoring request timed out. |
| ERR_TRTC_DISCONNECT_OTHER_ROOM_TIMEOUT | -3327 | The request to stop co-anchoring timed out. |
| ERR_TRTC_CONNECT_OTHER_ROOM_INVALID_PARAMETER | -3328 | Invalid parameter. |
| ERR_TRTC_CONNECT_OTHER_ROOM_AS_AUDIENCE | -3330 | The current user is an audience member and cannot request or stop cross-room communication. Please call ` switchRole ` to switch to an anchor first. |
| ERR_BGM_OPEN_FAILED | -4001 | Failed to open the file, such as invalid data found when processing input, ffmpeg protocol not found, etc. |
| ERR_BGM_DECODE_FAILED | -4002 | Audio file decoding failed. |
| ERR_BGM_OVER_LIMIT | -4003 | The number exceeds the limit, such as preloading two background music at the same time. |
| ERR_BGM_INVALID_OPERATION | -4004 | Invalid operation, such as calling a preload function after starting playback. |
| ERR_BGM_INVALID_PATH | -4005 | Invalid path, Please check whether the path you passed points to a legal music file. |
| ERR_BGM_INVALID_URL | -4006 | Invalid URL, Please use a browser to check whether the URL address you passed in can download the desired music file. If the operating system is iOS or MacOS, please ensure the use of HTTPS links.. |
| ERR_BGM_NO_AUDIO_STREAM | -4007 | No audio stream, Please confirm whether the file you passed is a legal audio file and whether the file is damaged. |
| ERR_BGM_FORMAT_NOT_SUPPORTED | -4008 | Unsupported format, Please confirm whether the file format you passed is a supported file format. The mobile version supports [mp3, aac, m4a, wav, ogg, mp4, mkv], and the desktop version supports [mp3, aac, m4a, wav, mp4, mkv]. |
| ERR_CONCURRENT_BGM_OVER_LIMIT | -4009 | The number of concurrent BGM playbacks has exceeded the limit. This error occurs when more than 10 BGMs are playing simultaneously. Please check the number of concurrent BGM playbacks.ConcurrentBgmOverLimit |
...
---
*Source: [https://trtc.io/document/54573](https://trtc.io/document/54573)*
